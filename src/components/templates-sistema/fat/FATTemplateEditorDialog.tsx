...
import {
  ...
  FAT_ITEM_TIPOS,
  type FATItemTipo,
  type FATTemplateItem,
  type FATTemplateSecao,
} from "@/lib/fat-templates.functions";
import { Textarea } from "@/components/ui/textarea";

const TIPO_LABEL: Record<FATItemTipo, string> = {
...
export function FATTemplateEditorDialog({
  templateId,
  open,
  onOpenChange,
}: {
  ...
  const getFn = useServerFn(getFATTemplate);
  const updTplFn = useServerFn(updateFATTemplate);
  const upSecFn = useServerFn(upsertFATSecao);
  const delSecFn = useServerFn(deleteFATSecao);
  const upItemFn = useServerFn(upsertFATItem);
  const delItemFn = useServerFn(deleteFATItem);

  const tplQ = useQuery({
    queryKey: ["fat-template", templateId],
    queryFn: () => getFn({ data: { id: templateId } }),
    enabled: open,
  });

  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["fat-template", templateId] });
    qc.invalidateQueries({ queryKey: ["fat-templates"] });
  };
...
function SecaoCard({
  ...
  sec: FATTemplateSecao;
  index: number;
  expanded: boolean;
  onToggle: () => void;
  onDeleteSec: () => void;
  onRenameSec: (t: string) => Promise<void>;
  onUpsertItem: (it: ItemUpsertPayload) => Promise<void>;
  onDeleteItem: (id: string) => Promise<void>;
  onChange: () => void;
}) {
  const [novoLabel, setNovoLabel] = useState("");
  const [novoTipo, setNovoTipo] = useState<FATItemTipo>("ok_nok_na");
        ...
        <div className="border-t border-[var(--bg-border)] p-3 space-y-2">
          ...
          <div className="flex gap-2 pt-2 border-t border-[var(--bg-border)]">
            <Input
              ...
              className="flex-1"
            />
            <Select value={novoTipo} onValueChange={(v) => setNovoTipo(v as FATItemTipo)}>
              <SelectTrigger className="w-[200px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {FAT_ITEM_TIPOS.map((t) => (
...
function ItemRow({
  ...
  it: FATTemplateItem;
  index: number;
  onUpsert: (it: ItemUpsertPayload) => Promise<void>;
  onDelete: () => void;
}) {
      ...
      {open && (
        <div className="border-t border-[var(--bg-border)] p-3 grid grid-cols-2 gap-3">
          <div>
            ...
            <Select
              ...
              onValueChange={async (v) => {
                await onUpsert({ ...it, tipo: v as FATItemTipo });
              }}
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <label className="flex items-center gap-2 text-[12px]">
              <Switch
                checked={it.permite_comentario}
                onCheckedChange={async (v) => {
                  await onUpsert({ ...it, permite_comentario: v });
                }}
              />
              Permite comentário
            </label>
            <label className="flex items-center gap-2 text-[12px]">
              <Switch
                checked={it.requer_foto_nok}
                onCheckedChange={async (v) => {
                  await onUpsert({ ...it, requer_foto_nok: v });
                }}
              />
              Requer foto se NOK
            </label>
