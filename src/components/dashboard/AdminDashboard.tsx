  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiCard label="Chamados fora do SLA" value={isLoading ? "…" : String(d?.kpis.chamadosSla ?? 0)} accent="danger" />
        <KpiCard label="Formulários > 72h" value={isLoading ? "…" : String(d?.kpis.formulariosPendentes ?? 0)} accent="warning" />
        <KpiCard label="OCs para aprovar" value={isLoading ? "…" : String(d?.kpis.ocsAprovar ?? 0)} accent="primary" />
        <KpiCard label="Erros enriquec. (24h)" value={isLoading ? "…" : String(d?.kpis.erros24h ?? 0)} accent="danger" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        ...
        />
        <DashboardCard title="Pendências por módulo" hint="Contagem de itens em fila" className="xl:col-span-2">
          <HeatStrip segments={d?.modulos ?? []} hint="Distribuição de pendências abertas por área." />
        </DashboardCard>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <DashboardCard title="Auditoria recente" hint="Últimos registros">
          {d?.auditoria.length ? (
            <ul className="space-y-3 text-sm">
              {d.auditoria.map((a) => (
                <li key={a.id} className="border-l border-[var(--bg-border)] pl-3">
                  <div className="text-[13px] font-medium text-[var(--text-primary)]">{a.actor}</div>
                  <div className="text-[11.5px] leading-5 text-[var(--text-muted)]">
                    {a.action} <span className="font-medium text-[var(--text-primary)]">{a.target}</span>
                  </div>
            ...
            <div className="py-6 text-center text-[12px] text-[var(--text-muted)]">Sem registros de auditoria.</div>
          )}
        </DashboardCard>
      </div>
    </DashboardShell>
