...
import { Loader2, Plus, Trash2, History, Save, Undo2, GripVertical, CheckSquare, ListTodo, CalendarClock, Info } from "lucide-react";
...
function SortableRow({ id, children }: { id: string; children: (h: { listeners: any; attributes: any }) => React.ReactNode }) {
...
export function TemplateEditorDialog({ templateId, open, onOpenChange }: Props) {
  ...
  // --- Checklist ---
  const [novoItem, setNovoItem] = useState({ secao: "Geral", titulo: "", obrigatorio: false, requer_arquivo: false });
  ...
  const addTar = useMutation({
    ...
    onSuccess: () => { setNovaTar({ titulo: "", dias: 7, role: "engineer" }); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const addEvt = useMutation({
    ...
    onSuccess: () => { setNovoEvt({ titulo: "", tipo: "marco", dias: 14 }); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const reorderItensMut = useMutation({
    mutationFn: (ids: string[]) => reorderItensFn({ data: { template_id: templateId, ordered_ids: ids } }),
    onSuccess: invalidate,
    onError: (e: Error) => toast.error(e.message),
  });
  const reorderTarefasMut = useMutation({
    mutationFn: (ids: string[]) => reorderTarefasFn({ data: { template_id: templateId, ordered_ids: ids } }),
    onSuccess: invalidate,
    onError: (e: Error) => toast.error(e.message),
  });
  const reorderEventosMut = useMutation({
    mutationFn: (ids: string[]) => reorderEventosFn({ data: { template_id: templateId, ordered_ids: ids } }),
    onSuccess: invalidate,
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <DialogContent className="max-w-5xl p-0 overflow-hidden">
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            {d && (
              <Badge className={`ml-1 border ${colorFor({ projeto: "bg-blue-500/15 text-blue-700 border-blue-500/30 dark:text-blue-300", atendimento: "bg-amber-500/15 text-amber-700 border-amber-500/30 dark:text-amber-300", instalacao: "bg-emerald-500/15 text-emerald-700 border-emerald-500/30 dark:text-emerald-300" }, d.template.tipo)}`}>
                {d.template.tipo}
              </Badge>
            )}
          </DialogTitle>
          {d && (
            <div className="flex flex-wrap items-center gap-4 pt-2 text-[11.5px] text-[var(--text-muted)]">
              <span>
                Criado por <strong className="text-[var(--text-secondary)]">{d.template.created_by_nome ?? "—"}</strong> em {formatDateTime(d.template.created_at)}
              </span>
              <span>
                Última edição por <strong className="text-[var(--text-secondary)]">{d.template.updated_by_nome ?? "—"}</strong> em {formatDateTime(d.template.updated_at)}
              </span>
              <Button
                size="sm"
                variant="outline"
        ...
        {d && (
          <Tabs defaultValue="checklist">
            ...
            <TabsContent value="checklist" className="space-y-3">
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(e) => handleDragEnd(d.itens, e, (ids) => reorderItensMut.mutate(ids))}>
                <SortableContext items={d.itens.map((i) => i.id)} strategy={verticalListSortingStrategy}>
                  <div className="space-y-1.5 max-h-[400px] overflow-auto pr-1">
                      ...
                      <SortableRow key={i.id} id={i.id}>
                        {({ listeners, attributes }) => (
                          <div className="group flex items-center gap-2 rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-2 text-[13px] transition hover:border-[var(--accent)]/40 hover:shadow-sm">
                            <button {...attributes} {...listeners} className="cursor-grab touch-none text-[var(--text-muted)] hover:text-[var(--text-primary)]" aria-label="Arrastar">
                              <GripVertical className="h-4 w-4" />
                            </button>
                            <Badge className={`border ${hashColor(i.secao)}`}>{i.secao}</Badge>
                            <span className="flex-1">{i.titulo}</span>
                            {i.obrigatorio && (
                              <Tooltip><TooltipTrigger asChild><Badge className="border bg-red-500/15 text-red-700 border-red-500/30 dark:text-red-300">obrig.</Badge></TooltipTrigger><TooltipContent>Item de preenchimento obrigatório.</TooltipContent></Tooltip>
                            )}
                            {i.requer_arquivo && (
                              <Tooltip><TooltipTrigger asChild><Badge className="border bg-indigo-500/15 text-indigo-700 border-indigo-500/30 dark:text-indigo-300">arquivo</Badge></TooltipTrigger><TooltipContent>Exige upload de arquivo.</TooltipContent></Tooltip>
                            )}
                    ...
                    {d.itens.length === 0 && (
                      <p className="text-center text-[12px] text-[var(--text-muted)] py-6">Sem itens. Adicione o primeiro abaixo.</p>
                    )}
                  </div>
                </SortableContext>
              </DndContext>
              <div className="grid grid-cols-12 gap-2 border-t border-[var(--bg-border)] pt-3">
                <Input
                  ...
                  onChange={(e) => setNovoItem((s) => ({ ...s, secao: capitalize(e.target.value) }))}
                />
                <Input
                  className="col-span-5"
                  placeholder="Título do item"
                  value={novoItem.titulo}
                  onChange={(e) => setNovoItem((s) => ({ ...s, titulo: capitalize(e.target.value) }))}
                />
                <label className="col-span-2 flex items-center gap-1.5 text-[12px]">
                  <Switch checked={novoItem.obrigatorio} onCheckedChange={(v) => setNovoItem((s) => ({ ...s, obrigatorio: v }))} /> obrig.
                </label>
                <label className="col-span-1 flex items-center gap-1.5 text-[12px]">
                  <Switch checked={novoItem.requer_arquivo} onCheckedChange={(v) => setNovoItem((s) => ({ ...s, requer_arquivo: v }))} /> arq.
                </label>
                <Button className="col-span-1" onClick={() => addItem.mutate()} disabled={!novoItem.titulo.trim() || addItem.isPending}>
            ...
            <TabsContent value="tarefas" className="space-y-3">
              <div className="flex items-center gap-2 text-[12px] text-[var(--text-muted)]">
                <Info className="h-3.5 w-3.5" />
                Tarefas geradas automaticamente quando o template é aplicado a um processo, com prazo calculado a partir da data de início.
              </div>
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(e) => handleDragEnd(d.tarefas, e, (ids) => reorderTarefasMut.mutate(ids))}>
                <SortableContext items={d.tarefas.map((t) => t.id)} strategy={verticalListSortingStrategy}>
                  <div className="space-y-1.5 max-h-[400px] overflow-auto pr-1">
                    {d.tarefas.map((t) => (
                          ...
                          <div className="group flex items-center gap-2 rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-2 text-[13px] transition hover:border-[var(--accent)]/40 hover:shadow-sm">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Badge className="border bg-slate-500/15 text-slate-700 border-slate-500/30 dark:text-slate-300">D+{t.dias_apos_inicio}</Badge>
                              </TooltipTrigger>
                              <TooltipContent>Dias após o início do processo em que a tarefa vence.</TooltipContent>
                            </Tooltip>
                            {t.responsavel_role && (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Badge className={`border ${colorFor(ROLE_COLORS, t.responsavel_role)}`}>{t.responsavel_role}</Badge>
                                </TooltipTrigger>
                                <TooltipContent>Perfil responsável por executar a tarefa.</TooltipContent>
                              </Tooltip>
                            )}
                            <Button size="sm" variant="ghost" onClick={() => removeTar.mutate(t.id)}>
                              <Trash2 className="h-3.5 w-3.5 text-[var(--danger)]" />
                            </Button>
                          </div>
                        )}
                    ...
                    {d.tarefas.length === 0 && (
                      <p className="text-center text-[12px] text-[var(--text-muted)] py-6">Sem tarefas.</p>
                    )}
                  </div>
                </SortableContext>
              </DndContext>
              <div className="grid grid-cols-12 gap-2 border-t border-[var(--bg-border)] pt-3">
                <Input
                  ...
                  onChange={(e) => setNovaTar((s) => ({ ...s, titulo: capitalize(e.target.value) }))}
                />
                <div className="col-span-2">
                  <Input
                    ...
                    placeholder="Dias"
                    value={novaTar.dias}
                    onChange={(e) => setNovaTar((s) => ({ ...s, dias: Number(e.target.value) || 0 }))}
                  />
                </div>
                <Select value={novaTar.role} onValueChange={(v) => setNovaTar((s) => ({ ...s, role: v as TemplateRole }))}>
                  <SelectTrigger className="col-span-3"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {ROLES.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Button className="col-span-1" onClick={() => addTar.mutate()} disabled={!novaTar.titulo.trim() || addTar.isPending}>
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            </TabsContent>
            ...
            <TabsContent value="eventos" className="space-y-3">
              <div className="flex items-center gap-2 text-[12px] text-[var(--text-muted)]">
                <Info className="h-3.5 w-3.5" />
                Marcos da timeline do processo (kickoff, FAT, embarque, etc.) com data calculada a partir do início.
              </div>
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(ev) => handleDragEnd(d.eventos, ev, (ids) => reorderEventosMut.mutate(ids))}>
                <SortableContext items={d.eventos.map((x) => x.id)} strategy={verticalListSortingStrategy}>
                  <div className="space-y-1.5 max-h-[400px] overflow-auto pr-1">
                    {d.eventos.map((e) => (
                      <SortableRow key={e.id} id={e.id}>
                        {({ listeners, attributes }) => (
                          <div className="group flex items-center gap-2 rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-2 text-[13px] transition hover:border-[var(--accent)]/40 hover:shadow-sm">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Badge className={`border ${colorFor(EVENTO_COLORS, e.tipo)}`}>{e.tipo}</Badge>
                              </TooltipTrigger>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Badge className="border bg-slate-500/15 text-slate-700 border-slate-500/30 dark:text-slate-300">D+{e.dias_apos_inicio}</Badge>
                              </TooltipTrigger>
                              <TooltipContent>Dias após o início do processo em que o evento acontece.</TooltipContent>
                            </Tooltip>
                            <Button size="sm" variant="ghost" onClick={() => removeEvt.mutate(e.id)}>
                              <Trash2 className="h-3.5 w-3.5 text-[var(--danger)]" />
                            </Button>
                          </div>
                        )}
                    ...
                    {d.eventos.length === 0 && (
                      <p className="text-center text-[12px] text-[var(--text-muted)] py-6">Sem eventos.</p>
                    )}
                  </div>
                </SortableContext>
              </DndContext>
              <div className="grid grid-cols-12 gap-2 border-t border-[var(--bg-border)] pt-3">
                <Input
                  ...
                  onChange={(e) => setNovoEvt((s) => ({ ...s, titulo: capitalize(e.target.value) }))}
                />
                <Select value={novoEvt.tipo} onValueChange={(v) => setNovoEvt((s) => ({ ...s, tipo: v as typeof novoEvt.tipo }))}>
                  <SelectTrigger className="col-span-3"><SelectValue /></SelectTrigger>
                  <SelectContent>
                ...
                <Input
                  ...
                  placeholder="Dias"
                  value={novoEvt.dias}
                  onChange={(e) => setNovoEvt((s) => ({ ...s, dias: Number(e.target.value) || 0 }))}
                />
                <Button className="col-span-1" onClick={() => addEvt.mutate()} disabled={!novoEvt.titulo.trim() || addEvt.isPending}>
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            </TabsContent>
            ...
            <TabsContent value="historico" className="space-y-2">
              ...
              {!versoesQ.isLoading && (versoesQ.data ?? []).length === 0 && (
                <p className="text-center text-[12px] text-[var(--text-muted)] py-4">
                  Sem versões salvas ainda. Toda alteração no template gera uma versão automaticamente.
                </p>
              )}
              <div className="max-h-[400px] space-y-1.5 overflow-auto">
                {(versoesQ.data ?? []).map((v) => (
                  ...
                  >
                    <Badge variant="outline" className="text-[11px]">v{v.versao}</Badge>
                    <div className="flex-1">
                      <div className="text-[var(--text-primary)]">{v.motivo ?? "(sem motivo)"}</div>
        ...
        <Dialog open={saveVerOpen} onOpenChange={setSaveVerOpen}>
          <DialogContent>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" onClick={() => setSaveVerOpen(false)}>Cancelar</Button>
              <Button
        ...
        <AlertDialog open={!!versaoRestore} onOpenChange={(o) => !o && setVersaoRestore(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                O conteúdo atual do template será substituído pelo desta versão.
                Uma nova versão com o estado atual é salva automaticamente antes da restauração.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
