...
async function audit(
  ...
  try {
    ...
    console.error("[entrevista_catalog_audit]", e);
  }
}
...
export const getSegmentoAdmin = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: perguntas, error: pErr } = await sb
      ...
      .select(
        "id, segmento_id, numero, ordem, formato, enunciado_pt, enunciado_es, enunciado_en, obrigatoria",
      )
    ...
    const respostasCnt = new Map<string, number>();
    if (pIds.length > 0) {
      const [opRes, rpRes] = await Promise.all([
        sb
          .from("entrevista_opcoes")
          .select("id, pergunta_id, ordem, label_pt, label_es, label_en, tem_descricao")
          .in("pergunta_id", pIds)
          .order("ordem", { ascending: true }),
    ...
    for (const o of opcoes) {
      ...
      arr.push(o);
      opByPerg.set(o.pergunta_id, arr);
    }
  ...
  .handler(async ({ data, context }) => {
    ...
    if (before) {
      await audit(sb, context.userId, {
        entity_type: "pergunta",
        entity_id: data.id,
        segmento_id: before.segmento_id,
        action: "delete",
        before,
        meta: { respostas: count ?? 0 },
      });
    }
    return { ok: true };
  });
...
export const reordenarPerguntas = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await audit(sb, context.userId, {
      entity_type: "pergunta",
      entity_id: data.segmento_id,
      segmento_id: data.segmento_id,
      action: "reorder",
      meta: { ordem: data.ordem },
    });
    return { ok: true };
  });
...
export const excluirOpcao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: before } = await sb
      .from("entrevista_opcoes")
      .select("*, entrevista_perguntas!inner(segmento_id)")
      .eq("id", data.id)
      .maybeSingle();
    ...
    if (before)
      await audit(sb, context.userId, {
        entity_type: "opcao",
        entity_id: data.id,
        segmento_id: before.entrevista_perguntas?.segmento_id ?? null,
        action: "delete",
        before,
      });
    return { ok: true };
...
export const reordenarOpcoes = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await audit(sb, context.userId, {
      entity_type: "opcao",
      entity_id: data.pergunta_id,
      segmento_id: perg?.segmento_id ?? null,
      action: "reorder",
      meta: { ordem: data.ordem },
    });
    return { ok: true };
