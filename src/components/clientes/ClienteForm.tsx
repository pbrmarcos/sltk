...
const DEFAULTS: ClienteInput = {
  ...
  socios: [],
  contatos: [
    { nome: "", cargo: "", email: "", telefone_ddi: "+55", telefone_numero: "", principal: true },
  ],
};
  ...
  return (
    <form
      onSubmit={form.handleSubmit(onSubmit, (errs) => {
        toast.error("Verifique os campos destacados — nenhum dado foi perdido.");
      ...
      <Section title="Identificação">
        <Grid>
          ...
          <Field
            ...
            error={docError}
            help={
              paisCfg?.documento_mascara && paisCfg.documento_mascara !== "livre"
                ? `Formato: ${paisCfg.documento_mascara}`
                : undefined
            }
          >
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    value={
                      paisCfg
                        ? formatDocumento(field.value, paisCfg.documento_mascara)
                        : field.value
                    }
          ...
          <Field
            ...
            error={form.formState.errors.email_corporativo?.message}
          >
          ...
          <Field label="Telefone corporativo">
            <div className="flex gap-2">
              <Input
                className="w-20"
                {...form.register("telefone_corporativo_ddi")}
                placeholder="+55"
              />
      ...
      {!isModal && (
        <Section title="Endereço">
          <Grid>
            ...
            <Field label="Cidade">
              <Input {...form.register("endereco_cidade")} />
            </Field>
            <Field label="Estado / Província">
              <Input {...form.register("endereco_estado")} />
            </Field>
            ...
            <Button
              ...
              onClick={() =>
                contatos.append({
                  ...
                  cargo: "",
                  email: "",
                  telefone_ddi: defaultDdi(paisCodigo),
                  telefone_numero: "",
                  principal: false,
                })
              }
          ...
          {(isModal ? contatos.fields.slice(0, 1) : contatos.fields).map((f, i) => (
            <div key={f.id} className="rounded-md border border-[var(--bg-border)] p-3">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {!isModal && (
                  <Field label="Cargo">
                    <Input {...form.register(`contatos.${i}.cargo` as const)} />
                  </Field>
                )}
                ...
                {!isModal && (
                  <Field label="DDI">
                    <Input
                      className="w-24"
                      {...form.register(`contatos.${i}.telefone_ddi` as const)}
                    />
                  </Field>
                )}
                <Field label="Telefone">
                  <Input {...form.register(`contatos.${i}.telefone_numero` as const)} />
                </Field>
        ...
        <CollapsibleSection
          ...
          right={
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                socios.append({ nome: "", qualificacao: "", desde: "" });
              }}
            >
              ...
              {socios.fields.map((f, i) => (
                <div key={f.id} className="rounded-md border border-[var(--bg-border)] p-3">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <Field label="Nome *">
                      <Input {...form.register(`socios.${i}.nome` as const)} />
                    </Field>
                    <Field label="Qualificação">
                      <Input {...form.register(`socios.${i}.qualificacao` as const)} />
                    </Field>
        ...
        <CollapsibleSection title="Dados fiscais (Brasil)" defaultOpen={false}>
          <Grid>
            ...
            <Field label="Situação cadastral">
              <Input {...form.register("situacao_cadastral")} />
            </Field>
            <Field label="Data da situação">
              <Input type="date" {...form.register("data_situacao")} />
            </Field>
            <Field label="Motivo da situação">
              <Input {...form.register("motivo_situacao")} />
            </Field>
            ...
            <Field label="Porte">
              <Input {...form.register("porte")} />
            </Field>
            <Field label="Natureza jurídica — código">
              <Input {...form.register("natureza_juridica_codigo")} />
            </Field>
            <Field label="Natureza jurídica — descrição">
              <Input {...form.register("natureza_juridica_descricao")} />
            </Field>
            <Field label="CNAE principal">
              <Input {...form.register("cnae_principal")} />
            </Field>
          </Grid>
        </CollapsibleSection>
      )}
        ...
        <CollapsibleSection title="Redes sociais (opcional)" defaultOpen={false}>
          <Grid>
            ...
            <Field label="Instagram">
              <Input placeholder="@usuario" {...form.register("social_instagram")} />
            </Field>
            <Field label="Facebook">
              <Input {...form.register("social_facebook")} />
            </Field>
            <Field label="Twitter / X">
              <Input placeholder="@usuario" {...form.register("social_twitter")} />
            </Field>
            <Field label="WhatsApp">
              <Input placeholder="+55 11 99999-9999" {...form.register("social_whatsapp")} />
            </Field>
            <Field label="Skype">
              <Input {...form.register("social_skype")} />
            </Field>
          </Grid>
        </CollapsibleSection>
      )}
