...
export const getManagerDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<DashboardData> => {
    ...
    const [oppsAllRes, oppsHotRes, processosRes, satRes, tarefasRes, clientesAtivosRes] =
      await Promise.all([
        sb
          .from("oportunidades")
          .select(
            "id, codigo, titulo, valor_estimado, probabilidade, pipeline_stage, expected_close_date, created_at, updated_at, lost_at, stage_entered_at, cliente_id, clientes(nome_fantasia, razao_social)",
          )
          .is("deleted_at", null)
        ...
        sb
          .from("oportunidades")
          .select(
            "id, codigo, titulo, valor_estimado, probabilidade, pipeline_stage, expected_close_date, cliente_id, clientes(nome_fantasia, razao_social)",
          )
          .is("deleted_at", null)
        ...
        sb
          .from("processos")
          .select(
            "id, codigo, titulo, stage, stage_entered_at, progresso, risco, previsao, cliente_id, clientes(nome_fantasia, razao_social)",
          )
          .is("deleted_at", null)
          .is("lost_at", null)
          .limit(500),
        sb
          .from("sat_relatorio")
          .select(
            "id, codigo, status, updated_at, cliente_id, clientes(nome_fantasia, razao_social)",
          )
          .is("deleted_at", null)
        ...
        sb
          ...
          .select(
            "id, titulo, prazo, processo_id, processos(codigo, cliente_id, clientes(nome_fantasia, razao_social))",
          )
          .eq("status", "aberta")
    ...
    const funnel = stages.map((stage) => {
      const items = opps.filter(
        (o) => o.pipeline_stage === stage && new Date(o.created_at) >= new Date(ninetyDaysAgo),
      );
      return {
        stage,
        count: items.length,
    ...
    const pipelineOpen = opps.filter(
      (o) => !["ganho", "perdido"].includes(o.pipeline_stage as string),
    );
    ...
    opps.forEach((o) => {
      ...
      if (e) {
        e.valor += Number(o.valor_estimado ?? 0);
        e.count += 1;
      }
    });
    ...
    const ganhoMesDelta =
      ganhoMesPrev > 0 ? ((ganhoMesCurr - ganhoMesPrev) / ganhoMesPrev) * 100 : 0;
    ...
    // Processos
    const procStagesAll: ProcessoStageKey[] = [
      "ETP",
      "Orçamento",
      "OC",
      "Eng. Mecânica",
    ...
    const processosByStage = procStagesAll.map((s) => ({
      stage: s,
      count: procStageCount.get(s) ?? 0,
    }));

    const processosSlaRisk = processos
      .map((p: any) => {
        const diasNaEtapa = Math.floor(
          (Date.now() - new Date(p.stage_entered_at).getTime()) / 86400000,
        );
        ...
        const score =
          ...
          (diasParaPrev !== null && diasParaPrev < 0
            ? 4
            : diasParaPrev !== null && diasParaPrev < 7
              ? 2
              : 0) +
    ...
    // SAT
    const satByStatus: Record<SatStatusKey, number> = {
      rascunho: 0,
      preenchendo: 0,
      assinado: 0,
      arquivado: 0,
...
function avgTicket(opps: any[], from: string, to: string) {
  const wins = opps.filter(
    (o) => o.pipeline_stage === "ganho" && o.updated_at >= from && o.updated_at < to,
  );
...
function avgCycle(opps: any[], from: string, to: string) {
  ...
  if (wins.length === 0) return 0;
  const days = wins.map(
    (o) => (new Date(o.updated_at).getTime() - new Date(o.created_at).getTime()) / 86400000,
  );
