...
import { KeyRound, Mail, ShieldAlert, RefreshCw } from "lucide-react";
...
function SuportePage() {
  ...
  if (!allowed) {
    return (
      <PageContainer>
        <PageHeader breadcrumbs={crumbs} title="Balcão de suporte" />
        <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-12 text-center">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Esta área é exclusiva para engenharia, gestão e administração.
          </p>
        </div>
      </PageContainer>
    );
  ...
  return <SuportePanel crumbs={crumbs} />;
}

function SuportePanel({ crumbs }: { crumbs: { label: string; href?: string }[] }) {
  ...
  return (
    <PageContainer>
      <PageHeader
        breadcrumbs={crumbs}
        title="Balcão de suporte"
        subtitle="Redefinir senhas e enviar links de recuperação para usuários operacionais."
        actions={
      ...
      <div className="mb-4 rounded-md border border-amber-300 bg-amber-50 p-3 text-[12px] text-amber-800">
