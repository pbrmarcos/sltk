  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!result?.ok) {
            const status = result?.error === "not_found" ? 404 : result?.error === "invalid" ? 400 : 410;
    ...
    const supabaseAdmin = await getCriticalClient();
            ...
            const [{ data: seg }, { data: prof }] = await Promise.all([
              supabaseAdmin.from("entrevista_segmentos").select("nome_pt").eq("id", ent.segmento_id).maybeSingle(),
