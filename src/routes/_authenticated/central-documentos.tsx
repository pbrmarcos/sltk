...
import { ShieldAlert, Save, History, Languages, RotateCcw, Loader2, GitCompare, AlertTriangle, CheckCircle2, ArrowUp, ArrowDown, Columns2, Square } from "lucide-react";
...
function AdminDocumentosPage() {
  ...
  if (role !== "admin" && role !== "manager") {
    return (
      <PageContainer>
        <PageHeader breadcrumbs={[{ label: "Documentos" }, { label: "Central de Documentos" }]} title="Central de Documentos" />
  ...
  return (
    <PageContainer>
      ...
      <Tabs defaultValue="blocos">
        ...
        <TabsContent value="blocos" className="mt-4"><BlocosTab /></TabsContent>
        <TabsContent value="layout" className="mt-4"><LayoutTab /></TabsContent>
        <TabsContent value="tipos" className="mt-4"><TiposTab /></TabsContent>
        <TabsContent value="rfq-gerados" className="mt-4"><RfqGeradosTab /></TabsContent>
        <TabsContent value="entrevistas" className="mt-4"><EntrevistasGeradasTab /></TabsContent>
      </Tabs>
    </PageContainer>
  );
}
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-3">
        <div className="mb-2 flex items-center justify-between gap-3">
          <Label className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Tipo de documento</Label>
          <div className="flex items-center gap-2">
            <Switch checked={autoTr} onCheckedChange={setAutoTr} id="auto-tr" />
            <Label htmlFor="auto-tr" className="cursor-pointer text-sm">Auto-traduzir ES/EN vazios ao salvar</Label>
          </div>
        </div>
      ...
      {tipos.map((t) => {
        ...
        return (
          ...
          >
            ...
            <span className="font-mono text-[10px] text-[var(--text-muted)]">{t.prefixo_codigo}</span>
          </button>
        );
      })}
    </div>
...
type EditState = {
  ...
  titulo_pt: string; titulo_es: string; titulo_en: string;
  comentario: string;
};
...
function BlocosList({ tipo, autoTr }: { tipo: string; autoTr: boolean }) {
  ...
  const handleToggleAtivo = async (b: any, ativo: boolean) => {
    try {
      await upd({ data: { id: b.id, ativo, comentario: ativo ? "bloco habilitado" : "bloco desabilitado" } });
  ...
  if (!q.data || q.data.length === 0) {
    return <div className="rounded-lg border border-dashed border-[var(--bg-border)] p-6 text-sm text-[var(--text-muted)]">Nenhum bloco para este tipo. Rode a migração de seed.</div>;
  }
    ...
    <>
      <div className="space-y-4">
        {(q.data ?? []).map((b: any) => {
          ...
          return (
            <div key={b.id} className={`rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-4 ${b.ativo === false ? "opacity-60" : ""}`}>
              <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="text-sm font-medium">
                    {b.nome}
                    {b.ativo === false ? <Badge variant="outline" className="ml-2">desabilitado</Badge> : null}
                  </h3>
                  <div className="text-xs text-[var(--text-muted)]">
                    <span className="font-mono">{b.codigo}</span> · ordem {b.ordem_padrao} · largura {b.largura ?? 100}%
                    {esEmpty ? <Badge variant="outline" className="ml-2">ES vazio</Badge> : null}
                    {enEmpty ? <Badge variant="outline" className="ml-2">EN vazio</Badge> : null}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="inline-flex overflow-hidden rounded-md border border-[var(--bg-border)]" role="group" aria-label="Largura do bloco">
                    <button
                      type="button"
                  ...
                  <Button size="sm" variant="outline" onClick={() => handleTranslate(b)} disabled={busy[b.id] === "tr"}>
                    {busy[b.id] === "tr"
                  ...
                  <TabsContent value={l} key={l} className="space-y-2 pt-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(ev) => setEditing((s) => ({ ...s, [b.id]: { ...s[b.id], [`titulo_${l}`]: ev.target.value } as EditState }))}
                      />
                    </div>
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={(ev) => setEditing((s) => ({ ...s, [b.id]: { ...s[b.id], [l]: ev.target.value } as EditState }))}
                      />
                    </div>
                  </TabsContent>
                ))}
              ...
              <div className="mt-3">
                <Label className="text-xs">Comentário desta alteração (opcional, registrado no histórico)</Label>
                <Input
                  placeholder="Ex.: ajuste cláusula 2 a pedido do jurídico"
                  value={editing[b.id]?.comentario ?? ""}
                  onChange={(ev) => setEditing((s) => ({ ...s, [b.id]: { ...s[b.id], comentario: ev.target.value } as EditState }))}
                />
              </div>
            </div>
          );
...
function HistoricoSheet({
  ...
  const handleRestore = async () => {
    if (!selected) return;
    try {
      await restore({ data: { versao_id: selected.id, comentario: restoreComentario || undefined } });
      toast.success(`Restaurado a partir de v${selected.versao_seq}. Tentativa registrada no audit log.`);
      setRestoreComentario("");
      onRestored();
      onClose();
    } catch (err) {
  ...
  const restoreImpact = useMemo(() => {
    ...
    const perLang = langs.map((l) => {
      ...
      for (const a of altered) { statusCur[a.name] = "altered"; statusVer[a.name] = "altered"; }
      for (const k of preserved) { statusCur[k] = "kept"; statusVer[k] = "kept"; }
      ...
      <SheetContent side="right" className="w-full max-w-4xl overflow-y-auto sm:max-w-4xl">
        ...
        {q.data && q.data.length === 0 && (
          <div className="mt-4 rounded-lg border border-dashed border-[var(--bg-border)] p-4 text-sm text-[var(--text-muted)]">
            Nenhuma versão registrada ainda. O histórico passa a ser gravado a partir da próxima alteração.
          </div>
        )}
            ...
            {(q.data || []).map((v: any) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="outline" className="text-[10px]">{v.acao}</Badge>
                </div>
                <div className="mt-1 text-[var(--text-muted)]">
                  {new Date(v.created_at).toLocaleString("pt-BR")}
                </div>
                <div className="truncate">{v.alterado_por_nome || "—"}</div>
                {v.comentario ? <div className="mt-1 truncate text-[var(--text-muted)]">"{v.comentario}"</div> : null}
              </button>
            ))}
          </div>
          <div>
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm">
                    <strong>v{selected.versao_seq}</strong> · {new Date(selected.created_at).toLocaleString("pt-BR")} ·{" "}
                    {selected.alterado_por_nome || "—"} · <Badge variant="outline">{selected.acao}</Badge>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button size="sm" variant="outline"><RotateCcw className="mr-2 h-4 w-4" /> Restaurar</Button>
                      </AlertDialogTrigger>
                      <AlertDialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Confira lado a lado como cada idioma ficará após a restauração. O conteúdo atual será salvo como nova versão e a tentativa será registrada no log de auditoria.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        {restoreImpact && (
                          <div className="space-y-4 text-sm">
                              ...
                              <div className="flex items-start gap-2 rounded-md border border-[var(--danger)]/40 bg-[color-mix(in_oklab,var(--danger)_10%,transparent)] p-3">
                                ...
                                <div className="space-y-1">
                                  <div className="font-medium text-[var(--danger)]">
                                    Atenção: {restoreImpact.totalRemoved} {restoreImpact.totalRemoved === 1 ? "placeholder será removido" : "placeholders serão removidos"}.
                                  </div>
                                  <div className="text-xs text-[var(--text-muted)]">
                                    Documentos que dependem desses {`{{placeholders}}`} podem ficar sem dados dinâmicos após a restauração.
                                  </div>
                                </div>
                              </div>
                            )}
                            <div className="grid grid-cols-2 gap-3 rounded-md border border-[var(--bg-border)] p-3 text-xs">
                              <div>
                                <div className="mb-1 font-medium uppercase tracking-wide text-[var(--text-muted)]">Bloco</div>
                                <div><span className="font-mono">{bloco?.codigo}</span> — {bloco?.nome}</div>
                              </div>
                              <div>
                                <div className="mb-1 font-medium uppercase tracking-wide text-[var(--text-muted)]">Idiomas afetados</div>
                                {restoreImpact.affected.length === 0 ? (
                                  <span className="text-[var(--text-muted)]">Nenhum — conteúdo já idêntico.</span>
                                ) : (
                                  <div className="flex flex-wrap gap-1">
                                    {restoreImpact.affected.map((a) => (
                                      <Badge key={a.lang} variant="outline">{a.lang.toUpperCase()}</Badge>
                                    ))}
                                  </div>
                                )}
                              </div>
                            ...
                            <div>
                              <Label className="text-xs">Comentário de auditoria (opcional, registrado no log)</Label>
                              <Input
                                ...
                                {(["pt", "es", "en"] as const).map((l) => {
                                  ...
                                  return (
                                    <TabsContent key={l} value={l} className="pt-3">
                                      <div className="grid grid-cols-2 gap-3">
                                        <div className="space-y-2">
                                          <div className="flex items-center gap-2 text-xs">
                                            ...
                                            {!info.changed && (
                                              <span className="text-[var(--success)]">sem alteração</span>
                                            )}
                                          </div>
                                          <div>
                                            <Label className="text-xs">Título</Label>
                                      ...
                                      {(info.removed.length > 0 || info.added.length > 0 || info.altered.length > 0 || info.preserved.length > 0) && (
                                        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border border-[var(--bg-border)] p-2 text-xs">
                                          <span className="flex items-center gap-1 text-[var(--text-muted)]">
                                            <span className="inline-block h-2 w-2 rounded-sm bg-[color-mix(in_oklab,var(--danger)_60%,transparent)]" /> removido
                                          </span>
                                          <span className="flex items-center gap-1 text-[var(--text-muted)]">
                                            <span className="inline-block h-2 w-2 rounded-sm bg-[color-mix(in_oklab,var(--warning,#d97706)_60%,transparent)]" /> alterado
                                          </span>
                                          <span className="flex items-center gap-1 text-[var(--text-muted)]">
                                      ...
                                      {info.altered.length > 0 && (
                                        <div className="mt-2 rounded-md border border-[color-mix(in_oklab,var(--warning,#d97706)_40%,transparent)] p-2 text-xs">
                                          <span className="text-[var(--warning,#d97706)]">Alterados (contagem de ocorrências):</span>{" "}
                                          {info.altered.map((a) => (
                                            <code key={a.name} className="mr-1 rounded bg-[color-mix(in_oklab,var(--warning,#d97706)_15%,transparent)] px-1 font-mono">
                                              {`{{${a.name}}}`} ({a.from}→{a.to})
                                            </code>
                                          ))}
                                        </div>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction
                            ...
                            className={restoreImpact && restoreImpact.totalRemoved > 0 ? "bg-[var(--danger)] hover:bg-[var(--danger)]/90" : ""}
                          >
                            {restoreImpact && restoreImpact.totalRemoved > 0 ? "Restaurar mesmo assim" : "Restaurar"}
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                      ...
                      {viewMode === "preview" ? (
                        <>
                          <div>
                            ...
                            <Input readOnly value={(selected[`conteudo_${l}`] || {}).titulo || ""} />
                          </div>
                          <div>
                            <Label className="text-xs">Texto (v{selected.versao_seq})</Label>
                            <Textarea readOnly rows={12} value={(selected[`conteudo_${l}`] || {}).texto || ""} />
                          </div>
                        </>
                      ) : (
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs text-[var(--text-muted)]">
                            <span>
                              Comparando <strong className="text-[var(--text-primary)]">v{selected.versao_seq}</strong>{" "}
                              → <strong className="text-[var(--text-primary)]">atual</strong>
                            </span>
                            {!hasChanges && <Badge variant="outline">sem alterações em {l.toUpperCase()}</Badge>}
                          </div>
                          <div className="overflow-hidden rounded-md border border-[var(--bg-border)] bg-[var(--bg-surface)] font-mono text-xs">
                            {diffParts.length === 0 ? (
                              <div className="p-3 text-[var(--text-muted)]">Sem conteúdo nesta língua.</div>
                            ) : (
                              diffParts.map((part, idx) => {
                                ...
                                return (
                                  <div key={idx}>
                                    {lines.map((ln, i) => (
                                      <div key={i} className={`whitespace-pre-wrap px-3 py-0.5 ${cls}`}>
                          ...
                          <p className="text-[10px] text-[var(--text-muted)]">
                            Linhas em vermelho serão recolocadas ao restaurar; linhas em verde (do atual) serão substituídas pelo conteúdo histórico.
                          </p>
                        </div>
                      )}
                    </TabsContent>
...
function LayoutTab() {
  const tiposQ = useQuery({ queryKey: ["doc-tipos"], queryFn: () => listDocumentoTipos() });
  const [tipo, setTipo] = useState<string>("orcamento");
  const q = useQuery({ queryKey: ["layout", tipo], queryFn: () => getLayoutConfig({ data: { tipo } }) });
  const upd = useServerFn(updateLayoutConfig);
  const [form, setForm] = useState({ accent_color: "#0B3D91", logo_url: "", empresa_nome: "", empresa_endereco: "", empresa_contato: "", rodape_extra: "" });
  ...
  useEffect(() => {
    if (q.data) {
      ...
      setForm({ accent_color: "#0B3D91", logo_url: "", empresa_nome: "", empresa_endereco: "", empresa_contato: "", rodape_extra: "" });
    }
  }, [q.data]);
  ...
  return (
    <div className="grid max-w-3xl gap-4 rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-6">
      <div>
        ...
        <TipoTabs
          tipos={(tiposQ.data ?? []) as Array<{ codigo: string; nome: string; prefixo_codigo: string }>}
          value={tipo}
          onChange={setTipo}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input value={form.empresa_endereco} onChange={(e) => setForm({ ...form, empresa_endereco: e.target.value })} />
        </div>
        <div>
          <Label>Contato (cabeçalho)</Label>
          <Input value={form.empresa_contato} onChange={(e) => setForm({ ...form, empresa_contato: e.target.value })} placeholder="contato@empresa.com · +55 11 9 0000-0000" />
        </div>
      </div>
            <div>
                ...
                <div className="flex flex-1 flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    {parsed.url ? (
                      ...
                      <span className="text-xs text-[var(--text-muted)]">Sem arquivo — usando logo padrão.</span>
                    )}
                  </div>
            ...
            <div>
              ...
              <Textarea rows={2} value={form.rodape_extra} onChange={(e) => setForm({ ...form, rodape_extra: e.target.value })} placeholder="Ex.: CNPJ 00.000.000/0001-00 · ISO 9001" />
              <p className="mt-1 text-xs text-[var(--text-muted)]">Aparece no rodapé entre o responsável e a paginação. Máx. 80 caracteres.</p>
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">
                A mesma logo (fundo claro), altura e espaço são aplicados ao cabeçalho de todos os PDFs gerados.
              </p>
            </div>



      <div><Button onClick={save}><Save className="mr-2 h-4 w-4" /> Salvar</Button></div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-[var(--bg-border)] bg-[var(--bg-surface)] p-6 text-sm">
      <div className="space-y-2">
        {(tiposQ.data ?? []).map((t) => (
          <div key={t.codigo} className="flex items-center justify-between">
            <div>
              <span className="font-medium">{t.nome}</span>{" "}
              <span className="ml-2 font-mono text-xs text-[var(--text-muted)]">{t.codigo} · {t.prefixo_codigo}</span>
            </div>
      ...
      <p className="mt-4 text-xs text-[var(--text-muted)]">
        Tipos novos são provisionados automaticamente quando o módulo correspondente gera seu primeiro documento.
      </p>
    </div>
  );
}
