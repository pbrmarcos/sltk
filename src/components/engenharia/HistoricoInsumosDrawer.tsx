...
import {
  History, FileSpreadsheet, Pencil, Trash2, Plus, Download,
  Send, Package, Rocket,
} from "lucide-react";

const TIPO_META: Record<string, { label: string; icon: any; tone: string }> = {
  import_excel: { label: "Import Excel", icon: FileSpreadsheet, tone: "bg-emerald-100 text-emerald-800" },
  ...
  envio_aprovacao: { label: "Envio p/ aprovação", icon: Send, tone: "bg-violet-100 text-violet-800" },
  estoque_alterado: { label: "Estoque", icon: Package, tone: "bg-teal-100 text-teal-800" },
  liberado_producao: { label: "Liberado produção", icon: Rocket, tone: "bg-fuchsia-100 text-fuchsia-800" },
};
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-lg">
        <SheetHeader>
          ...
          <SheetDescription>Importações, edições, estoque e aprovações registrados por usuário.</SheetDescription>
        </SheetHeader>
