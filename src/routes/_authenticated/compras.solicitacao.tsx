...
export const Route = createFileRoute("/_authenticated/compras/solicitacao")({
  validateSearch: (s: Record<string, unknown>): SolicitacaoSearch => ({
    ...
    origem:
      s.origem === "eqp" || s.origem === "projeto" || s.origem === "todos" ? s.origem : undefined,
  }),
...
function SolicitacoesPage() {
  ...
  const insumosQ = useQuery({
    queryKey: [
      "compras",
      "solicitacoes",
      { q, status, criticidade, disciplina, origem, equipamentoFilter },
    ],
    queryFn: () =>
      listInsumos({
        data: {
  ...
  const criticos = rows.filter(
    (r) => r.criticidade === "critica" || r.criticidade === "alta",
  ).length;
      ...
      <Tabs defaultValue="lista" className="w-full">
        ...
        <TabsContent value="lista" className="mt-4">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            <Select
              value={origem}
              onValueChange={(v) => setOrigem(v as "todos" | "eqp" | "projeto")}
            >
              <SelectTrigger className="w-[160px]">
                <SelectValue />
              </SelectTrigger>
            ...
            {equipamentoFilter ? (
              <Badge
                variant="outline"
                className="border-blue-200 bg-blue-50 text-[11px] font-normal text-blue-700"
              >
                  ...
                  <TableRow>
                    <TableCell colSpan={8}>
                      <TableEmpty
                        ...
                        description="Quando engenharia listar insumos nos projetos eles aparecem aqui para o Compras."
                      />
                    </TableCell>
                  </TableRow>
                ) : (
                      ...
                      <TableCell className="text-xs">
                        {r.equipamento_disciplina ? (
                          <>
                            <Badge
                              variant="outline"
                              className="border-blue-200 bg-blue-50 text-[10px] font-normal text-blue-700"
                            >
                              Aba EQP
                            </Badge>
                          ...
                          <>
                            <Badge
                              variant="outline"
                              className="border-[var(--bg-border)] bg-[var(--bg-elevated)] text-[10px] font-normal text-[var(--text-secondary)]"
                            >
                              Projeto formal
                            </Badge>
                            <div className="mt-1 font-medium">
