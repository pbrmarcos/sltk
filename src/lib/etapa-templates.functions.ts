...
export const DISCIPLINAS_PROJETO = [
  "mecanico",
  "eletrico",
  "automacao",
  "montagem",
...
export const listEtapaTemplates = createServerFn({ method: "POST" })
  ...
  .inputValidator((i: unknown) =>
    z
      .object({ q: z.string().optional(), publicado: z.enum(["todos", "sim", "nao"]).optional() })
      .parse(i ?? {}),
  )
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: itens }, { data: bom }, { data: versoes }] = await Promise.all([
      ...
      sb
        ...
        .select("id, versao, comentario, actor_nome, created_at")
        .eq("template_id", data.id)
...
export const duplicateEtapaTemplate = createServerFn({ method: "POST" })
  ...
  .inputValidator((i: unknown) =>
    z
      .object({ id: z.string().uuid(), novoNome: z.string().min(2), novoSlug: z.string().min(2) })
      .parse(i),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase as AnySb;
...
export const applyEtapaTemplateBulk = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await sb
      .from("etapa_template")
      .update({ versao_atual: proximaVersao })
      .eq("id", data.templateId);
    await writeAudit(
      sb,
      context.userId!,
...
export const publishEtapaTemplate = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ id: z.string().uuid(), comentario: z.string().optional() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase as AnySb;
    await requireManagerRole(sb, context.userId!);
    ...
    await sb
      .from("etapa_template")
      .update({ publicado: true, versao_atual: proximaVersao })
      .eq("id", data.id);
    await writeAudit(sb, context.userId!, "publish", "etapa_template", data.id, null, {
      versao: proximaVersao,
    });
...
export const revertEtapaTemplateVersao = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ templateId: z.string().uuid(), versao: z.number().int() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase as AnySb;
    await requireManagerRole(sb, context.userId!);
    ...
    await writeAudit(sb, context.userId!, "revert", "etapa_template", data.templateId, antes, {
      revertidoPara: data.versao,
    });
