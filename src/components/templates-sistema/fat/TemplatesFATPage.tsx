...
import {
  listFATTemplates,
  novaVersaoFATTemplate,
  setFATTemplateAtivo,
  archiveFATTemplate,
  type FATTemplateLite,
...
import { FATTemplateEditorDialog } from "./FATTemplateEditorDialog";
...
export function TemplatesFATPage({ view = "ativos" }: { view?: "ativos" | "arquivados" }) {
  const qc = useQueryClient();
  const listFn = useServerFn(listFATTemplates);
  const novaFn = useServerFn(novaVersaoFATTemplate);
  const ativoFn = useServerFn(setFATTemplateAtivo);
  const arquivarFn = useServerFn(archiveFATTemplate);

  const [editId, setEditId] = useState<string | null>(null);
  const [archTpl, setArchTpl] = useState<FATTemplateLite | null>(null);

  const listQ = useQuery({
    queryKey: ["fat-templates"],
    queryFn: () => listFn(),
  });

  const novaMut = useMutation({
    mutationFn: (base?: FATTemplateLite) =>
      novaFn({
        data: {
          base_id: base?.id,
          nome: base ? `${base.nome} (cópia)` : "Novo template FAT",
