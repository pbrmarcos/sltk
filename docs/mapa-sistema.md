...
- **Vitest não roda nenhum módulo que importe `zod`**: `TypeError: z.object is not a function` — afeta hoje `permissoes.test.ts` e `equipamento-pagina.shared.test.ts`. Zod funciona normalmente fora do Vitest (confirmado rodando direto via `bun`); é algo na interação Vitest/Bun/zod v4 (dual-package ESM/CJS) neste ambiente, nunca investigado a fundo. Cresce a cada novo teste que precise validar um schema Zod — vale investigar quando aparecer de novo. **Atualização**: numa rodada seguinte o bug parou de se manifestar (possivelmente cache do Bun) e `permissoes.test.ts` rodou de verdade — revelando 2 asserções pré-existentes que esperam o erro cru do Postgres vazar ("db down"/"boom" no texto da exceção), comportamento que a correção de segurança do Tier 1 (`friendlyDbError`) já bloqueia de propósito. O teste ficou desatualizado, não o código — vale corrigir as 2 asserções pra checar a mensagem amigável em vez da crua.
