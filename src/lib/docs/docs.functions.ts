...
export const listDocumentos = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    let q = (context.supabase as any)
      .from("documentos")
      .select(
        "id, codigo, tipo_codigo, titulo, status, versao, idiomas_gerados, cliente_id, oportunidade_id, responsavel_id, created_at, updated_at",
      )
    ...
    const clienteIds = Array.from(
      new Set((docs || []).map((d: any) => d.cliente_id).filter(Boolean)),
    );
...
export async function generateOrcamentoImpl(
  ...
  const context = { userId } as { userId: string };
  {
    ...
    const [
      { data: blocos, error: bErr },
      { data: layout, error: lErr },
      { data: tipoRow, error: tErr },
    ] = await Promise.all([
      supabase
        .from("documento_blocos")
        .select("*")
    ...
    // Validação
    const selected =
      data.payload.blocos_selecionados.length > 0
    ...
    const imagensCache = new Map<
      string,
      { dataUrl: string; bytes: Uint8Array; mime: string; ext: string }
    >();
    ...
    const revisaoMeta = data.documento_id
      ...
      : null;
    const versionPayload = revisaoMeta
      ? { ...data.payload, _revisao_meta: revisaoMeta }
      : data.payload;
    ...
    try {
      ...
      try {
        if (imagensCache.size > 0) {
          ...
          const { data: cliFull } = await supabase
            ...
            .maybeSingle();
          const clienteFolder =
            `${cliFull?.codigo || clienteCod} - ${(cliFull?.razao_social || "").slice(0, 80)}`.trim();
          const imgFolder = await ensurePath([
            "Clientes",
            clienteFolder,
          ...
          const slug = (s: string) =>
            (s || "")
              ...
              .replace(/[^a-z0-9]+/g, "-")
              .replace(/^-+|-+$/g, "")
              .slice(0, 60) || "equipamento";
          await Promise.all(
            (data.payload.equipamentos || []).map(async (eq, i) => {
      ...
      await supabase.from("documentos").update({ drive_sync_error: drive_error }).eq("id", docId);
    }

    return {
      ok: true as const,
      documento_id: docId as string,
      codigo,
      versao,
      arquivos,
      drive_synced,
...
export const generateOrcamento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) =>
    generateOrcamentoImpl(context.supabase, context.userId, data),
  );
  ...
  switch (acao) {
    case "submeter":
      if (!(isOwner || isAdmin || isManager))
        throw new Error("Apenas o autor, manager ou admin podem submeter para revisão.");
      return;
    case "aprovar":
    case "rejeitar":
    case "publicar":
    case "arquivar":
      if (!(isAdmin || isManager))
        throw new Error("Apenas manager ou admin podem executar esta ação.");
      return;
    case "reabrir":
...
const TRANSITIONS: Record<AprovAcao, { from: DocStatus[]; to: DocStatus }> = {
  ...
  arquivar: {
    from: ["rascunho", "em_revisao", "aprovado", "publicado", "emitido"],
    to: "arquivado",
  },
...
async function executeApprovalAction(
  ...
  if (rule.to === "aprovado" && acao === "aprovar") {
    try {
      await autoCreateEquipamentoFromOrcamento(
        supabaseAdmin,
        documento_id,
        context.userId,
        actorNome,
...
export async function autoCreateEquipamentoFromOrcamento(
  ...
  for (const eq of equipamentos) {
    ...
    else if (modeloLower.match(/sache|pouch|termo|encart|encaix|empacot/))
      categoria = "embalagem_secundaria";

    const { data: novoEq } = await supabaseAdmin
      .from("cliente_equipamentos")
      .insert({
    ...
    if (novoEq?.id) {
      try {
        await supabaseAdmin.rpc("import_etapas_do_template", {
          _eq_id: novoEq.id,
          _tipo_slug: templateSlug,
        });
      } catch (err) {
...
async function uploadAndSignImpl(
  supabase: any,
  documento_id: string,
  userId: string,
  userNome: string,
  ...
  const clienteFolder =
    `${cli?.codigo || doc.cliente_id} - ${(cli?.razao_social || "").slice(0, 80)}`.trim();
...
export const submitForReview = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "submeter", data.comentario),
  );
...
export const approveDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "aprovar", data.comentario),
  );
...
export const rejectDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "rejeitar", data.comentario),
  );
...
export const publishDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "publicar", data.comentario),
  );
...
export const archiveDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "arquivar", data.comentario),
  );
...
export const reopenDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator(aprovacaoInput)
  .handler(({ data, context }) =>
    executeApprovalAction(context, data.documento_id, "reabrir", data.comentario),
  );
...
async function ensureFatTipo(supabaseAdmin: any) {
  const { data } = await supabaseAdmin
    .from("documento_tipos")
    .select("codigo, prefixo_codigo")
    .eq("codigo", "fat_report")
    .maybeSingle();
  if (data) return data;
...
export async function generateFatDocumentInternal(args: {
  ...
  if (docId) {
    ...
    const { data: ins, error: iErr } = await (supabaseAdmin as any)
      .from("documentos")
      .insert({
        codigo,
        tipo_codigo: "fat_report",
        cliente_id: fat.cliente_id,
        ...
        versao,
        responsavel_id: fat.inspetor_id || actorId,
        payload: payloadSnapshot,
        blocos: [],
        idiomas_gerados: idiomas,
        created_by: actorId,
...
export async function generateSatDocumentInternal(args: {
  ...
  const [{ data: cli }, { data: proc }, { data: secoes }, { data: anexos }] = await Promise.all([
    (supabaseAdmin as any)
      .from("clientes")
      .select("id, codigo, razao_social")
      .eq("id", sat.cliente_id)
      .maybeSingle(),
    sat.processo_id
      ? (supabaseAdmin as any)
          .from("processos")
          .select("codigo, titulo")
          .eq("id", sat.processo_id)
          .maybeSingle()
      : Promise.resolve({ data: null }),
    (supabaseAdmin as any)
    ...
    .map((sec: any) => ({
      ...
      itens: ((sec.sat_template_item as any[]) || [])
        ...
        .map((it) => ({
          id: it.id,
          label: it.label,
          tipo: it.tipo as SatItemTipo,
          valor: dados[it.id] ?? null,
          comentario: null,
        })),
    }));
  ...
  const satPayload: SatPdfPayload = {
    ...
    sat: {
      codigo: sat.codigo,
      local_endereco: sat.local_endereco,
      periodo_de: sat.periodo_de,
      periodo_ate: sat.periodo_ate,
      motivos_viagem: sat.motivos_viagem || [],
      observacoes: sat.observacoes,
      status: sat.status,
      assinatura_tecnico_url: sigTec.url,
      assinatura_cliente_url: sigCli.url,
      assinatura_tecnico_nome: sigTec.nome,
      assinatura_cliente_nome: sigCli.nome,
    },
    tecnicos,
    equipamentos,
    secoes: secoesPayload,
    anexos: (anexos || []).map((a: any) => ({
      nome: a.nome_original || a.nome_final,
      tipo: a.tipo_anexo || "—",
  ...
  if (docId) {
    ...
    const { data: ins, error: iErr } = await (supabaseAdmin as any)
      ...
      .insert({
        ...
        versao,
        responsavel_id: sat.created_by || actorId,
        payload: payloadSnapshot,
        blocos: [],
        idiomas_gerados: idiomas,
        created_by: actorId,
...
export const getOrcamentoForEdit = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const rawPayload = (lastVersion?.payload || doc.payload) as
      | OrcamentoPayload
      | (OrcamentoPayload & { _revisao_meta?: unknown });
