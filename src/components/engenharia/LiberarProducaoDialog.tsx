...
import {
  resumoLiberacaoProducao,
  liberarEquipamentoProducao,
...
function buildResumoRows(r: any, inicio: string, fim: string, obs: string) {
  ...
  rows.push([
    "Cliente",
    `${eq?.clientes?.codigo ?? ""} ${eq?.clientes?.razao_social ?? ""}`.trim(),
  ]);
  ...
  for (const [disc, s] of Object.entries(r?.etapasByDisc ?? {}) as Array<[string, any]>) {
    rows.push([
      disc,
      `${s.concluido ?? 0} / ${s.em_progresso ?? 0} / ${s.bloqueado ?? 0} / ${s.nao_iniciado ?? 0}`,
    ]);
  }
  rows.push(["", ""]);
  rows.push(["Insumos", ""]);
  ...
  for (const [s, n] of Object.entries(r?.insumosStatus ?? {}))
    rows.push([`Status: ${s}`, String(n)]);
  return { eq, rows };
}
...
export function LiberarProducaoDialog({
  ...
  const eq = r?.equipamento;
  const disciplinasEtapas = Object.entries(r?.etapasByDisc ?? {}) as Array<
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Confira o resumo abaixo. Após liberar, o pacote técnico é congelado, a Ordem de Montagem
            é gerada e Compras passa a poder emitir Ordens de Compra dos insumos aprovados.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="rounded-md border bg-[var(--bg-elevated)] p-3">
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                {eq?.clientes?.razao_social}{" "}
                {eq?.clientes?.codigo ? `· ${eq?.clientes?.codigo}` : ""}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {totais?.rascunhoCount > 0 && (
                <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-900">
                  ...
                  <span>
                    <b>{totais.rascunhoCount}</b>{" "}
                    {totais.rascunhoCount === 1 ? "insumo" : "insumos"} ainda em rascunho. Você pode
                    liberar mesmo assim, mas eles não gerarão OC até serem aprovados.
                  </span>
                </div>
              )}
              {r.aprovacoesPend > 0 && (
                <div className="flex items-start gap-2 rounded-md border border-rose-200 bg-rose-50 p-2 text-xs text-rose-900">
                  <AlertTriangle className="h-4 w-4 shrink-0" />
                  <span>
                    <b>{r.aprovacoesPend}</b>{" "}
                    {r.aprovacoesPend === 1 ? "aprovação pendente" : "aprovações pendentes"} —
                    ...
                    return (
                      <div key={disc} className="rounded border p-2">
                        ...
                        <div className="mt-1 flex flex-wrap gap-1 text-[10px]">
                          {(
                            ["concluido", "em_progresso", "bloqueado", "nao_iniciado"] as const
                          ).map((k) =>
                            (s[k] ?? 0) > 0 ? (
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div className="rounded border p-2">
                  ...
                  <p className="text-sm font-semibold tabular-nums">
                    {fmtMoeda(totais?.totalEstimado ?? 0)}
                  </p>
                </div>
                <div className="rounded border border-emerald-200 bg-emerald-50 p-2">
                  ...
                  <p className="text-sm font-semibold tabular-nums text-emerald-900">
                    {fmtMoeda(totais?.totalEstoque ?? 0)}
                  </p>
                </div>
                <div className="rounded border border-blue-200 bg-blue-50 p-2">
                  <p className="text-[10px] uppercase text-blue-700">A comprar</p>
                  <p className="text-sm font-semibold tabular-nums text-blue-900">
                    {fmtMoeda(totais?.totalAComprar ?? 0)}
                  </p>
                </div>
              </div>
        ...
        <DialogFooter className="gap-2 sm:justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={requestClose} disabled={mut.isPending}>
              Cancelar
            </Button>
            <Button
              onClick={() => mut.mutate()}
