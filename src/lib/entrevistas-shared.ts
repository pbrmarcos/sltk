...
export const I18N: Record<Idioma, {
  next: string; back: string; submit: string; sending: string;
  progress: string; required: string; describe: string;
  language: string; success_title: string; success_body: string;
  select_country: string; not_found: string; expired: string;
  optional_text: string; number_placeholder: string;
  review_title: string; review_subtitle: string; edit: string;
  no_answer: string; invalid_email: string; invalid_whatsapp: string;
  resume_notice: string; resume_clear: string; contact_step_label: string;
  retrying: string; sent_partial: string; unsaved_warn: string;
}> = {
  pt: {
    next: "Avançar", back: "Voltar", submit: "Enviar respostas", sending: "Enviando…",
    progress: "de", required: "Obrigatório", describe: "Descreva",
    language: "Idioma", success_title: "Recebemos suas respostas!",
    success_body: "Obrigado. Nossa equipe entrará em contato em breve.",
    select_country: "Selecione o país", not_found: "Entrevista não encontrada ou expirada.",
    expired: "Este link expirou.", optional_text: "Sua resposta",
    ...
    edit: "Editar", no_answer: "— sem resposta —",
    invalid_email: "E-mail inválido.", invalid_whatsapp: "WhatsApp inválido (inclua DDI, ex.: +55).",
    resume_notice: "Retomamos do ponto em que você parou.",
    resume_clear: "Recomeçar", contact_step_label: "Contato",
    retrying: "Tentando novamente…", sent_partial: "Falha ao enviar. Tentando novamente…",
    unsaved_warn: "Você tem respostas não enviadas. Deseja realmente sair?",
  },
  es: {
    next: "Siguiente", back: "Volver", submit: "Enviar respuestas", sending: "Enviando…",
    progress: "de", required: "Obligatorio", describe: "Describa",
    language: "Idioma", success_title: "¡Recibimos tus respuestas!",
    success_body: "Gracias. Nuestro equipo se pondrá en contacto en breve.",
    select_country: "Selecciona el país", not_found: "Entrevista no encontrada o vencida.",
    expired: "Este enlace ha expirado.", optional_text: "Tu respuesta",
    ...
    edit: "Editar", no_answer: "— sin respuesta —",
    invalid_email: "Correo inválido.", invalid_whatsapp: "WhatsApp inválido (incluye el código de país, ej.: +54).",
    resume_notice: "Retomamos donde lo dejaste.",
    resume_clear: "Reiniciar", contact_step_label: "Contacto",
    retrying: "Reintentando…", sent_partial: "Fallo al enviar. Reintentando…",
    unsaved_warn: "Tienes respuestas sin enviar. ¿Seguro que quieres salir?",
  },
  en: {
    next: "Next", back: "Back", submit: "Submit answers", sending: "Sending…",
    progress: "of", required: "Required", describe: "Describe",
    language: "Language", success_title: "We received your answers!",
    success_body: "Thanks. Our team will reach out shortly.",
    select_country: "Select country", not_found: "Interview not found or expired.",
    expired: "This link has expired.", optional_text: "Your answer",
    ...
    edit: "Edit", no_answer: "— no answer —",
    invalid_email: "Invalid e-mail.", invalid_whatsapp: "Invalid WhatsApp (include country code, e.g. +1).",
    resume_notice: "We resumed from where you left off.",
    resume_clear: "Start over", contact_step_label: "Contact",
    retrying: "Retrying…", sent_partial: "Send failed. Retrying…",
    unsaved_warn: "You have unsent answers. Are you sure you want to leave?",
  },
};


export function pickLang<T extends { nome_pt: string; nome_es?: string | null; nome_en?: string | null }>(
  obj: T | null | undefined, lang: Idioma,
): string {
  if (!obj) return "";
...
export function pickText(pt: string, es: string | null | undefined, en: string | null | undefined, lang: Idioma): string {
  const PT = pt ?? "";
  const ES = es ?? "";
  const EN = en ?? "";
...
export function maskWhatsapp(raw: string): string {
  ...
  const cc = digits.slice(0, digits.length > 11 ? digits.length - 10 : Math.max(1, digits.length - 10));
...
export function flagEmoji(iso2: string): string {
  ...
  return String.fromCodePoint(base + iso2.toUpperCase().charCodeAt(0) - A) +
...
export type OpcaoLite = {
  ...
  label_pt: string; label_es: string | null; label_en: string | null;
  tem_descricao: boolean;
};

const RE_NAME = /(nome\s+do\s+respons|nombre\s+del?\s+respons|responsible.*name|name\s*of\s*respons)/i;
const RE_EMAIL = /(^|\b)(e-?mail|correo|email)\b/i;
const RE_WHATS = /whats?app|whatsapp/i;

function isNameLabel(o: OpcaoLite) { return RE_NAME.test(o.label_pt) || RE_NAME.test(o.label_es || "") || RE_NAME.test(o.label_en || ""); }
function isEmailLabel(o: OpcaoLite) { return RE_EMAIL.test(o.label_pt) || RE_EMAIL.test(o.label_es || "") || RE_EMAIL.test(o.label_en || ""); }
function isWhatsLabel(o: OpcaoLite) { return RE_WHATS.test(o.label_pt) || RE_WHATS.test(o.label_es || "") || RE_WHATS.test(o.label_en || ""); }
function isFieldLabel(o: OpcaoLite) { return isNameLabel(o) || isEmailLabel(o) || isWhatsLabel(o); }

export type ContactMatrixGroup<T extends OpcaoLite = OpcaoLite> = { role: T; nome?: T; email?: T; whatsapp?: T };

/** Detects role+Nome+E-mail+Whatsapp option groups. Returns null when the list isn't a matrix. */
export function groupContactMatrix<T extends OpcaoLite>(opcoes: T[]): ContactMatrixGroup<T>[] | null {
