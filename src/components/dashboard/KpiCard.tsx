...
export function KpiCard({ label, value, hint, delta, deltaSuffix = "%", invertDelta, accent = "primary" }: Props) {
  const up = (delta ?? 0) > 0.5;
  const down = (delta ?? 0) < -0.5;
  const positive = invertDelta ? down : up;
  const negative = invertDelta ? up : down;
  const accentColor =
    accent === "success" ? "var(--success, #22c55e)" :
    accent === "warning" ? "var(--warning, #f59e0b)" :
    accent === "danger"  ? "var(--danger,  #ef4444)" :
                           "var(--primary)";
  ...
  return (
    <div className="relative overflow-hidden rounded-[var(--radius-lg)] border border-[var(--bg-border)] bg-[var(--bg-surface)] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] transition-colors hover:border-[var(--text-muted)]/40">
        ...
        {delta !== undefined && (
          <span
            className={cn(
              ...
              !positive && !negative && "border-[var(--bg-border)] bg-[var(--bg-elevated)] text-[var(--text-muted)]"
            )}
          >
      ...
      <div className="mt-3 text-2xl font-semibold tracking-tight text-[var(--text-primary)] tabular-nums">{value}</div>
