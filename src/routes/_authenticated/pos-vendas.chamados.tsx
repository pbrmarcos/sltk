    ...
    queryFn: () =>
      listFn({
        data: {
          ...
          date_from: search.date_from ? new Date(search.date_from + "T00:00:00").toISOString() : null,
    ...
    <PageContainer>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <Input
          ...
          onKeyDown={(e) => { if (e.key === "Enter") update({ q: qInput }); }}
          onBlur={() => { if (qInput !== search.q) update({ q: qInput }); }}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <Input
          ...
          onKeyDown={(e) => { if (e.key === "Enter") update({ cliente_q: cliInput }); }}
          onBlur={() => { if (cliInput !== search.cliente_q) update({ cliente_q: cliInput }); }}
        ...
        <Input
          ...
          onKeyDown={(e) => { if (e.key === "Enter") update({ cnpj: cnpjInput }); }}
          onBlur={() => { if (cnpjInput !== search.cnpj) update({ cnpj: cnpjInput }); }}
        ...
        <Input
          ...
          onKeyDown={(e) => { if (e.key === "Enter") update({ mensagem_q: msgInput }); }}
          onBlur={() => { if (msgInput !== search.mensagem_q) update({ mensagem_q: msgInput }); }}
        ...
        {(search.cliente_q || search.cnpj || search.mensagem_q || search.date_from || search.date_to) ? (
          <Button
            size="sm"
            variant="ghost"
            onClick={() => update({ cliente_q: "", cnpj: "", mensagem_q: "", date_from: "", date_to: "" })}
          >
            Limpar avançado
          </Button>
        ) : null}
        ...
        <Table>
          ...
          <TableBody>
            {query.isLoading ? (
              ...
              <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-8">Nenhum chamado encontrado.</TableCell></TableRow>
            ) : (
              rows.map((r: any) => {
                const pendente = r.ultima_mensagem_por === "visitante" && r.status !== "resolvido" && r.status !== "arquivado";
                return (
                  <TableRow key={r.id} className="cursor-pointer">
                    <TableCell className="font-mono">
                      <Link to="/pos-vendas/chamados/$id" params={{ id: r.id }} className="text-primary hover:underline">
                        {r.codigo}
                      </Link>
                      {pendente ? <span className="ml-2 inline-block rounded-full bg-rose-100 text-rose-700 text-[10px] px-2 py-0.5">novo</span> : null}
                    </TableCell>
                    <TableCell><ChamadoOrigemBadge origem={r.origem} /></TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div>{r.assunto ?? "—"}</div>
                      {r.numero_serie ? <div className="text-xs text-muted-foreground">sn {r.numero_serie}</div> : null}
                    </TableCell>
                    <TableCell><ChamadoPrioridadeBadge prioridade={r.prioridade} /></TableCell>
                    <TableCell><ChamadoStatusBadge status={r.status} /></TableCell>
                    <TableCell className="text-sm">{r.atendente_nome ?? <span className="text-muted-foreground">—</span>}</TableCell>
                    <TableCell>
                      <SlaClock slaAt={r.sla_resposta_at} finalizedAt={r.first_response_at} compact />
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {r.ultima_mensagem_em ? new Date(r.ultima_mensagem_em).toLocaleString() : new Date(r.created_at).toLocaleString()}
                    </TableCell>
                  </TableRow>
                );
              })
      ...
      <div className="flex items-center justify-between mt-3">
        <Button variant="outline" size="sm" onClick={() => navigate({ search: (p: typeof search) => ({ ...p, page: Math.max(1, p.page - 1) }) })} disabled={search.page === 1}>Anterior</Button>
        <div className="text-sm text-muted-foreground">Página {search.page}</div>
        <Button variant="outline" size="sm" onClick={() => navigate({ search: (p: typeof search) => ({ ...p, page: p.page + 1 }) })} disabled={rows.length < 25}>Próxima</Button>
      </div>
    </PageContainer>
  );
}
