...
import { ShieldAlert, Search, Plus, Copy, Eye, EyeOff, Trash2, ExternalLink, ClipboardList } from "lucide-react";
    ...
    return (
      <PageContainer>
        <div className="mx-auto max-w-md rounded-lg border border-border bg-card p-6 text-center">
          ...
          <p className="text-xs text-muted-foreground">Somente admin, manager ou engineer.</p>
        </div>
      </PageContainer>
    );
  }
    ...
    <PageContainer>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por nome, slug ou família…" className="pl-8" />
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!isLoading && rows.length === 0 && (
              <tr><td colSpan={7} className="px-3 py-6 text-center text-muted-foreground">Nenhum template.</td></tr>
            )}
            {rows.map((r: any) => (
              <tr key={r.id} className="border-b last:border-b-0 hover:bg-muted/20">
                ...
                <td className="px-3 py-2">{r.familia ? <Badge variant="secondary">{r.familia}</Badge> : "—"}</td>
                ...
                <td className="px-3 py-2">
                  {r.publicado ? (
                    <Badge className="bg-emerald-500/15 text-emerald-700 hover:bg-emerald-500/20">Publicado</Badge>
                  ) : (
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setDup({ id: r.id, nome: r.nome })}>
                      <Copy className="h-4 w-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => togglePubMut.mutate({ id: r.id, publicado: !r.publicado })}>
      ...
      <NovoDialog open={openNovo} onOpenChange={setOpenNovo} onCreated={() => qc.invalidateQueries({ queryKey: ["etapa-tpl-list"] })} />
      {dup && (
        <DuplicarDialog
          origem={dup}
...
function NovoDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (v: boolean) => void; onCreated: () => void }) {
  const [nome, setNome] = useState("");
  const [slug, setSlug] = useState("");
  const [familia, setFamilia] = useState("");
  const [descricao, setDescricao] = useState("");
  const [tipoId, setTipoId] = useState<string>("");
  const { data: tipos = [] } = useQuery({ queryKey: ["rfq-tipos-tpl"], queryFn: () => listRfqTiposForTemplate() });
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      onCreated();
      onOpenChange(false);
      setNome(""); setSlug(""); setFamilia(""); setDescricao(""); setTipoId("");
    },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Novo template de etapas</DialogTitle></DialogHeader>
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))} placeholder="ex: envasadora-liquidos" />
          </div>
          <div>
          ...
          <div>
            <Label>Família</Label>
            <Input value={familia} onChange={(e) => setFamilia(e.target.value)} placeholder="ex: empacotamento" />
          </div>
          <div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => createMut.mutate()} disabled={!nome || !slug || createMut.isPending}>Criar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function DuplicarDialog({ origem, onOpenChange, onDuplicated }: { origem: { id: string; nome: string }; onOpenChange: (v: boolean) => void; onDuplicated: () => void }) {
  const [nome, setNome] = useState(`${origem.nome} (cópia)`);
  const [slug, setSlug] = useState("");
  const mut = useMutation({
    mutationFn: () => duplicateEtapaTemplate({ data: { id: origem.id, novoNome: nome, novoSlug: slug } }),
    onSuccess: () => {
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Duplicar template</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label>Novo nome</Label><Input value={nome} onChange={(e) => setNome(e.target.value)} /></div>
          <div><Label>Novo slug</Label><Input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))} /></div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancelar</Button>
          <Button onClick={() => mut.mutate()} disabled={!nome || !slug || mut.isPending}>Duplicar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
