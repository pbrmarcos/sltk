...
async function runUser(browser, { email, role }) {
  ...
  const result = {
    email,
    role,
    allowedTotal: cfg.allowed.length + config.common.length,
    login: false,
    divergences: [],
  };
  ...
  const missing = (cfg.sidebar ?? []).filter(
    (label) => !sidebar.some((s) => s.toLowerCase().includes(label.toLowerCase())),
  );
  if (missing.length) {
    ...
    result.divergences.push({
      path: "/dashboard",
      category: "sidebar",
      message: `Missing sidebar items: ${missing.join(", ")}`,
      screenshot: s,
    });
  }
  ...
  for (const route of allow) {
    ...
    if (status && status >= 500) flags.push({ category: "route-5xx", message: `HTTP ${status}` });
      ...
      flags.push({ category: "route-4xx", message: `HTTP ${status}` });
    if (/Algo deu errado|Módulo não encontrado/i.test(body))
      flags.push({ category: "boundary", message: "Error boundary visible" });
    if (/Acesso restrito|Sem permissão|Acesso negado/i.test(body))
      flags.push({
        category: "module-blocked",
        message: "Guard de módulo bloqueou uma rota permitida ao papel",
      });
    ...
    if (realConsole.length)
      flags.push({ category: "console", message: realConsole[0].slice(0, 240) });
  ...
  for (const route of cfg.denied ?? []) {
    try {
      ...
      const gated =
        finalUrl !== route ||
        /Sem permissão|Acesso negado|Acesso restrito|Not authorized/i.test(body);
      if (!gated) {
        const s = await shot(page, role, slug, "leak_" + route.replaceAll("/", "_"));
        result.divergences.push({
          path: route,
          category: "denied-leak",
          message: `Route reachable but should be denied for role ${role}`,
          screenshot: s,
        });
      }
    } catch {
      /* denied via error is fine */
    }
  }
...
const pkg = JSON.parse(
  await readFile(path.join(process.cwd(), "package.json"), "utf8").catch(() => "{}"),
);
await writeFile(
