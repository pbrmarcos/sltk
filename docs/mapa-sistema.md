...
### Rename completo RFQ → Checklist/Cotação + varredura final de permissões (2026-09-06) — ✅ CONCLUÍDO

Projeto à parte planejado formalmente (Plan Mode) na sessão anterior, executado em 3 frentes com `tsc`/`lint`/`test`/`build` limpos a cada commit:

- **Frente 1 — RFQ → Checklist (Comercial)**: as 4 tabelas (`rfq_formulario_tipo/link/submissao/submissao_anexo`), as colunas FK (`processo_templates.rfq_tipo_id`, `oportunidades.rfq_submissao_id`), o event_key de e-mail (`form.rfq.recebida` → `form.checklist.recebida`), a coluna `form_inbox_status.entity_type` e a pasta do Drive ("RFQ" → "Checklist") viraram `checklist_*`/"Checklist" no banco (migration `20260906140000`). No código: `rfq.functions.ts`/`rfq.shared.ts`/`rfq-etp.functions.ts` → `checklist.*`, pasta `components/rfq/` → `components/checklist/`, rotas públicas `/api/public/rfq/*` → `/api/public/checklist/*`, e todos os ~15 arquivos consumidores atualizados.
- **Frente 2 — RFQ → Cotação (Compras)**: conceito totalmente diferente (pedido de cotação a fornecedor) que também usava "rfq" no código. Tabela `insumo_rfq_envios` (+ enums `insumo_rfq_canal`/`insumo_rfq_status`) → `insumo_cotacao_*` (migration `20260906150000`), 2 event_keys de e-mail renomeados, arquivos `pdf-rfq.tsx`/`compras-rfq-docs.functions.ts`/`RfqGeradosTab.tsx`/`RfqTooltip.tsx` renomeados. Essa confusão de nomes já tinha vazado pra ~10 pontos da UI de Compras (botões e textos dizendo "Checklist" quando o fluxo real gera uma Cotação) — todos corrigidos.
- **Frente 3, item final — varredura de guardas de papel fixo**: `entrevistas.functions.ts` e `mineracao.functions.ts` trocaram lista de papel fixa por `assertCanAccessModule(..., "comercial")`; vários handlers de `entrevistas.functions.ts` não tinham guarda nenhuma (só RLS) e ganharam a checagem. `equipamento-etp-anexos.functions.ts`/`equipamento-etp-historico.functions.ts` trocaram RPC inline de `has_role` por `assertCanAccessModule(..., "engenharia")`. `insumo-aprovacoes.functions.ts` foi avaliado e mantido como está — os dois conjuntos de papéis (quem solicita vs. quem decide aprovação de OC) são regra de negócio deliberada, não matriz disfarçada.
- **Nota de verificação**: as duas migrations desta rodada não puderam ser testadas contra Postgres real neste ambiente (sem `psql`/Docker) — só confirmam quando aplicadas em staging/produção via `/admin/migrations`. Os tipos gerados do Supabase (`integrations/supabase/types.ts`) ficam desatualizados até alguém regerá-los depois de aplicar as migrations; o único ponto do código que dependia disso sem guarda de tipo (`checklist-etp.functions.ts`) recebeu cast explícito documentado, sem impacto em runtime.
