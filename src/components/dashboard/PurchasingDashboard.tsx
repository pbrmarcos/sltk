  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiCard label="OCs para aprovar" value={isLoading ? "…" : String(d?.kpis.ocsAprovar ?? 0)} accent="warning" />
        <KpiCard label="Cotações abertas" value={isLoading ? "…" : String(d?.kpis.cotacoesAbertas ?? 0)} accent="primary" />
        <KpiCard label="Gasto no mês" value={isLoading ? "…" : fmtBRL(d?.kpis.gastoMes ?? 0)} />
        <KpiCard label="Insumos aguardando" value={isLoading ? "…" : String(d?.kpis.insumosAguardando ?? 0)} accent="primary" />
      </div>
